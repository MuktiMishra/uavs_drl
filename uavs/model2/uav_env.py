...
def train():
    ...
    for episode in range(num_episodes):
        ...
        stuck_steps = 0
        last_pos = env.uav_pos
        f2_debug, f3_debug, f4_debug = 0, 0, 0

        for step in range(max_steps):
            if env.uav_pos == last_pos:
                stuck_steps += 1
            else:
                stuck_steps = 0
            last_pos = env.uav_pos
            ...
            if stuck_steps >= 2:
                ...
                action_for_log_prob, log_prob, value = agent.select_action(state)
                stuck_steps = 0
            else:
                action, log_prob, value = agent.select_action(state)
        ...
        agent.update(rewards, log_probs, values)
        print(f"Episode {episode+1}/{num_episodes} - Total Reward: {total_reward:.4f} | f2: {f2_debug:.4f}, f3: {f3_debug:.4f}, f4: {f4_debug:.4f}")
