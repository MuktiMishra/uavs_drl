...
class DiagnosticUAVEnv(gym.Env):
    ...
    def __init__(self, debug=False):
        ...
        # --- IMPROVEMENT: Increased max_steps to allow more time for exploration/rescue ---
        self.max_steps = 250  # Increased from 150
    ...
    def reset(self):
        ...
        self.total_reward_components = {
            ...
            'time_penalty': 0, 'completion_bonus': 0, 'no_move_penalty': 0 # --- IMPROVEMENT: Added new component ---
        }
    ...
    def _calculate_detailed_reward(self, victim_rescued, prev_pos, was_new_area):
        ...
        --- IMPROVEMENT: Adjusted reward weights for exploration and penalties. ---
        """
        reward_components = {
            'victim_rescue': 0, 'exploration': 0, 'movement': 0,
            'energy_penalty': 0, 'risk_penalty': 0, 'stuck_penalty': 0,
            'time_penalty': 0, 'completion_bonus': 0, 'no_move_penalty': 0
        }
        ...
        # --- IMPROVEMENT: Increased exploration reward ---
        if was_new_area:
            reward_components['exploration'] = 2.5 # Increased from 1.0

        # 3. Movement reward/penalty: Encourage moving, penalize staying still
        if self.uav_pos != prev_pos:
            reward_components['movement'] = 0.1
            reward_components['no_move_penalty'] = 0 # No penalty if moved
        else:
            reward_components['movement'] = -0.1 # Still a mild movement penalty for not changing coordinates
            # --- IMPROVEMENT: Stronger immediate penalty for not moving ---
            reward_components['no_move_penalty'] = -0.75 # Stronger immediate penalty for not moving

        # 4. Energy penalty: Mild penalty for using energy
        energy_cost_this_step = 0.003 + (0.002 if self.uav_pos != prev_pos else 0) + (0.005 if victim_rescued else 0)
        reward_components['energy_penalty'] = -0.5 * energy_cost_this_step # Use the actual energy consumed for scaling
        ...
        # --- IMPROVEMENT: Significantly increased stuck penalty ---
        if len(set(self.position_history)) <= 2 and self.step_count > 5: # Only apply after initial movement
            reward_components['stuck_penalty'] = -1.5 # Increased from -0.5
...
class DiagnosticCNNActorCritic(nn.Module):
    ...
    --- IMPROVEMENT: Added BatchNorm2d layers for training stability. ---
    """
    def __init__(self, input_channels=8, num_actions=4):
        ...
        self.bn1 = nn.BatchNorm2d(8) # --- IMPROVEMENT: Batch Normalization ---
        self.conv2 = nn.Conv2d(8, 8, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm2d(8) # --- IMPROVEMENT: Batch Normalization ---
    ...
    def forward(self, x):
        ...
        """
        x = torch.relu(self.bn1(self.conv1(x))) # --- IMPROVEMENT: Apply BN after conv, before ReLU ---
        x = torch.relu(self.bn2(self.conv2(x))) # --- IMPROVEMENT: Apply BN after conv, before ReLU ---
...
class DiagnosticPPOAgent:
    ...
    def __init__(self, model, lr=1e-4, gamma=0.99, eps_clip=0.2, k_epochs=4, entropy_coef=0.02):
        ...
        # --- HYPERPARAMETER SUGGESTION: Learning Rate (lr) ---
        # Current: lr=1e-4. Consider trying 2e-4 or 3e-4 if agent learns too slowly.
        # But if losses explode or become erratic, reduce it.
        ...
        # --- HYPERPARAMETER SUGGESTION: k_epochs ---
        # Current: k_epochs=4. Consider increasing to 8 or 10 if you need more policy updates per batch.
        self.k_epochs = k_epochs # Number of epochs to update the policy
        # --- HYPERPARAMETER SUGGESTION: Entropy Coefficient (entropy_coef) ---
        # Current: entropy_coef=0.02. This is CRITICAL for exploration.
        # If the agent is getting stuck, increase this significantly (e.g., 0.03, 0.05, or even 0.1).
        # Too high makes it too random, too low makes it exploit too quickly.
...
def detailed_train_with_loss_tracking():
    ...
    agent = DiagnosticPPOAgent(model, lr=1e-4, entropy_coef=0.03) 
...
def plot_training_results(results_path="model_output/diagnostic_training_results_with_losses.csv",
    ...
    fig, (ax1, ax2, ax3, ax4, ax5 , ax6) = plt.subplots(6, 1, figsize=(12, 30))
    ...
    ax4.grid(True)
     ...
     # --- IMPROVEMENT: Victims Rescued on ax5 ---
    ax5.plot(df['episode'], df['victims_rescued'], label='Victims Rescued', color='orange')
    ax5.set_title(f'Victims Rescued (Episodes {start_episode}-{end_episode})')
    ax5.set_xlabel('Episode')
    ax5.set_ylabel('Count')
    ax5.legend()
    ax5.grid(True)

    # --- IMPROVEMENT: Coverage on ax6 ---
    ax6.plot(df['episode'], df['coverage_ratio'] * (env.grid_height * env.grid_width),
             label=f'Coverage (scaled to {env.grid_height * env.grid_width} cells)', color='cyan', alpha=0.5)
    ax6.set_title(f'Coverage (Episodes {start_episode}-{end_episode})')
    ax6.set_xlabel('Episode')
    ax6.set_ylabel('Scaled Cells Covered')
    ax6.legend()
