...
class UAVEnvironment(gym.Env):
    def __init__(self, height=15, width=15, init_battery=1.0, total_time=1.0, source=(0, 0)):
        ...
        # Action space: 4 complex actions
        ...
        # Battery parameters
        self.base_energy_per_action = 0.005  # base energy cost per step
        self.battery_degradation = 0.001     # degradation cost per step
        self.env_noise_mean = 0
        self.env_noise_std = 0.0012           # stochastic noise
        
        self.task_energy_costs = {0: 0.002, 1: 0.01, 2: 0.015, 3: 0.005}  # sample values
    ...
    def step(self, action):
        # Define movement for action a₁ and a₄ (for simplicity, say reroute = random move)
        move_dict = {
            0: (0, 1),   # Explore Path: move right for demo
            3: (random.choice([-1,0,1]), random.choice([-1,0,1])),  # Reroute random move
        }
